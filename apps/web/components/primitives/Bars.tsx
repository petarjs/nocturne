...
export function Bars({
  ...
  const max = Math.max(1, ...categories.map((c) => Math.abs(c.value)));
  ...
  return (
    <div className={`flex h-full w-full items-end gap-3 ${className}`}>
      {categories.map((c, i) => {
        ...
        const pct = Math.max(2, (Math.abs(c.value) / max) * 100);
        const barColor = c.value < 0 ? "var(--n-negative)" : color;
        return (
          <div key={`${c.label}-${i}`} className="flex h-full min-w-0 flex-1 flex-col items-center justify-end gap-1.5">
            {showValues && (
              <span
            ...
            <motion.div
              ...
              style={{ background: barColor, transformOrigin: "bottom", boxShadow: emphasized ? `0 0 16px ${barColor}66` : "none" }}
              initial={false}
              animate={{
                height: `${pct}%`,
                opacity: emphasized
